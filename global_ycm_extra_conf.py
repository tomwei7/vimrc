...
DIR_OF_THIRD_PARTY = p.join( DIR_OF_THIS_SCRIPT, 'plugged/YouCompleteMe/third_party/ycmd', 'third_party' )
...
def PlatformIOProject(filename):
  ...
  while start_dir not in ('', '/') :
    if not p.exists(p.join(start_dir, 'platformio.ini')):
  ...
  if project_root is None:
    return None
  with open(p.join(project_root, '.gcc-flags.json')) as fp:
    gcc_flags = json.load(fp)
  if 'gccDefaultCppFlags' in gcc_flags:
    flags = gcc_flags['gccDefaultCppFlags'].split()
  else:
    flags = gcc_flags['gccDefaultCFlags'].split()
  gcc_include_paths = gcc_flags['gccIncludePaths'].split(',')
  flags += list(map(lambda path: '-I'+path.strip('\''), gcc_include_paths))
  return flags
...
if __name__ == "__main__":
  print(PlatformIOProject('/Users/weicheng/Playground/testplatformio/src/main.cpp'))
